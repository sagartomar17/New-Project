...
const ExtractionTable = ({
  ...
  const requestedByFilter = (
    <Dropdown
      ...
      onChange={(e) => { setFilterBy(e.value); setFirst(0) }}
  ...
  const requestIdFilter = (
    <span className={styles.searchWrap}>
      ...
      <InputText
        ...
        onChange={(e) => { setFilterRequestId(e.target.value); setFirst(0) }}
  ...
  const fileFilter = (
    <span className={styles.searchWrap}>
      ...
      <InputText
        value={filterFile}
        onChange={(e) => { setFilterFile(e.target.value); setFirst(0) }}
  ...
  const dateFilter = (
    <Calendar
      value={filterDate}
      onChange={(e) => { setFilterDate(e.value); setFirst(0) }}
      ...
      showButtonBar
      onClearButtonClick={() => { setFilterDate(null); setFirst(0) }}
    />
  )
  ...
  const statusFilter = (
    <Dropdown
      ...
      onChange={(e) => { setFilterStatus(e.value); setFirst(0) }}
      placeholder="Select One"
      className={styles.filterDropdown}
  ...
  const paginatorLeft = (
    <div className={styles.paginatorLeft}>
      ...
      <Dropdown
        value={rows}
        options={ROWS_PER_PAGE_OPTIONS.map((v) => ({ label: String(v), value: v }))}
        onChange={(e) => { setRows(e.value); setFirst(0) }}
  ...
  const emptyMessage = (
    <div className={styles.emptyState}>
      <i className="pi pi-inbox" style={{ fontSize: '2.5rem', color: '#CBD5E0', marginBottom: '12px' }} />
      <p style={{ color: '#718096', fontSize: '14px' }}>No extraction requests found.</p>
      <p style={{ color: '#A0AEC0', fontSize: '12px', marginTop: '4px' }}>Try adjusting your filters.</p>
    </div>
  )
