    ...
    <header className={styles.navbar}>
      ...
      <div className={styles.right}>
        ...
        >
          {/* trigger */}
          <div
            className={`${styles.userArea} ${dropOpen ? styles.userAreaActive : ''}`}
            onClick={() => setDropOpen((prev) => !prev)}
            role="button"
            tabIndex={0}
            aria-haspopup="true"
            aria-expanded={dropOpen}
            onKeyDown={(e) => e.key === 'Enter' && setDropOpen((prev) => !prev)}
          >
            <div className={styles.avatar}>
              <i className="pi pi-user" style={{ color: '#fff', fontSize: '14px' }} />
            </div>
            <i
              className={`pi pi-chevron-down ${styles.chevron} ${dropOpen ? styles.chevronUp : ''}`}
            />
          </div>
